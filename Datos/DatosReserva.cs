...
namespace Datos
{
    public class DatosReserva
    {
        ...
        public Reserva ValidaReserva(string idReserva)
        {
            
            Reserva res = null;
            SqlConnection connection = Conexion.openConection();
            string query = "select idReservas, estado, idInstalacion, dni, fecha,hora from reservas where reservas.idReservas = @Id;";

            using (SqlCommand command = new SqlCommand(query, connection))
            {
                command.Parameters.AddWithValue("@Id",int.Parse(idReserva));
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
